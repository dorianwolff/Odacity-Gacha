...
public class Quest
{
    ...
    public static void StartQuest(List<Character> characterCollection)
    {
        QuestFight(characterCollection); //Temporary
    }
    
    public static void QuestFight(List<Character> characterCollection)
    {
        ...
        Enemies.InitializeQuestEnemies();

        foreach (Enemies enemy in Enemies.questEnemies)
        {
    ...
    public static void StartFight(List<Character> characterCollection)
    {
        ...
        foreach (Enemies enemys in Enemies.questEnemies)
        {
            if (enemys.HP <= 0)
            {
                enemys.HP = 0;
    ...
    public static void DisplayEnemyTurn(Enemies enemy)
    {
        ...
        foreach (Enemies enem in Enemies.questEnemies)
        {
            if (enem.HP <= 0)
            {
                enem.IsAlive = false;
        ...
        string se1 =CalculateCubes(Enemies.questEnemies[0].MaxHP,Enemies.questEnemies[0].HP,true);
        if (!Enemies.questEnemies[0].IsAlive)
            se1 += "   \u001b[31m" + Enemies.questEnemies[0].Name+"\u001b[0m";
        else if (Enemies.questEnemies[0]==enemy) //active
        {
            se1 += "   \u001b[32m" + Enemies.questEnemies[0].Name+"\u001b[0m";
        }
        else
        {
            se1 += "   " + Enemies.questEnemies[0].Name;
        }
        if (Enemies.questEnemies[0].IsBuffed)
            enemyBuffsAndDebuffs1 +="\u001b[34m⚔\u001b[0m ";
        if (Enemies.questEnemies[0].IsPoisonned)
            enemyBuffsAndDebuffs1 += "\u001b[32m☠"+ "\u001b[0m ";
        if (Enemies.questEnemies[0].IsStuned)
        ...
        string se2="";
        if (Enemies.questEnemies.Count>1)
        {
            se2 = CalculateCubes(Enemies.questEnemies[1].MaxHP, Enemies.questEnemies[1].HP,true);
            if (!Enemies.questEnemies[1].IsAlive)
                se2 += "   \u001b[31m" + Enemies.questEnemies[1].Name+"\u001b[0m";
            else if (Enemies.questEnemies[1]==enemy) //active
            {
                se2 += "   \u001b[32m" + Enemies.questEnemies[1].Name+"\u001b[0m";
            }
            else
            {
                se2 += "   " + Enemies.questEnemies[1].Name;
            }
            if (Enemies.questEnemies[1].IsBuffed)
                enemyBuffsAndDebuffs2 +="\u001b[34m⚔\u001b[0m ";
            if (Enemies.questEnemies[1].IsPoisonned)
                enemyBuffsAndDebuffs2 += "\u001b[32m☠"+ "\u001b[0m ";
            if (Enemies.questEnemies[1].IsStuned)
                enemyBuffsAndDebuffs2 += "\u001b[33m꩜"+ "\u001b[0m";
            
        }
        string se3="";
        if (Enemies.questEnemies.Count>2)
        {
            se3 = CalculateCubes(Enemies.questEnemies[2].MaxHP, Enemies.questEnemies[2].HP,true);
            if (!Enemies.questEnemies[2].IsAlive)
                se3 += "   \u001b[31m" + Enemies.questEnemies[2].Name+"\u001b[0m";
            else if (Enemies.questEnemies[2]==enemy) //active
            {
                se3 += "   \u001b[32m" + Enemies.questEnemies[2].Name+"\u001b[0m";
            }
            else
            {
                se3 += "   " + Enemies.questEnemies[2].Name;
            }
            if (Enemies.questEnemies[2].IsBuffed)
                enemyBuffsAndDebuffs3 +="\u001b[34m⚔\u001b[0m ";
            if (Enemies.questEnemies[2].IsPoisonned)
                enemyBuffsAndDebuffs3 += "\u001b[32m☠"+ "\u001b[0m ";
            if (Enemies.questEnemies[2].IsStuned)
                enemyBuffsAndDebuffs3 += "\u001b[33m꩜"+ "\u001b[0m";
        }
        string se4="";
        if (Enemies.questEnemies.Count>3)
        {
            se4 = CalculateCubes(Enemies.questEnemies[3].MaxHP, Enemies.questEnemies[3].HP,true);
            if (!Enemies.questEnemies[3].IsAlive)
                se4 += "   \u001b[31m" + Enemies.questEnemies[3].Name+"\u001b[0m";
            else if (Enemies.questEnemies[3]==enemy) //active
            {
                se4 += "   \u001b[32m" + Enemies.questEnemies[3].Name+"\u001b[0m";
            }
            else
            {
                se4 += "   " + Enemies.questEnemies[3].Name;
            }
            if (Enemies.questEnemies[3].IsBuffed)
                enemyBuffsAndDebuffs4 +="\u001b[34m⚔\u001b[0m ";
            if (Enemies.questEnemies[3].IsPoisonned)
                enemyBuffsAndDebuffs4 += "\u001b[32m☠"+ "\u001b[0m ";
            if (Enemies.questEnemies[3].IsStuned)
        ...
        //TODO
        Console.WriteLine("\n" +
                          "                         \u001b[31mCombat\u001b[0m - \u001b[36mQuest Level "+Dungeon.questStageClear+"\u001b[0m\n");
        Console.WriteLine("/-----------------------------------------------------------------------\\");
        ...
        else
        {
                ...
                else
            {
                if (enemy.UltSkill.BuffEffect == "" && enemy.UltSkill.DebuffEffect == "") //Normal attack
                {
                    ...
                    {
                        foreach (Enemies character in Enemies.questEnemies) //AOE
                        {
                ...
                else if (enemy.UltSkill.DebuffEffect == "") //Buff attack
                {
                    ...
                    else
                    {
                        if (enemy.UltSkill.BuffEffect == "dodge")
                        {
                            foreach (Enemies enem in Enemies.questEnemies) //AOE
                            {
    ...
    public static void DisplayCharacterTurn(Character character,List<Character> characterCollection)
    {
        ...
        {
            DisplayEnemyInfo(Enemies.questEnemies[Arrow-1],0);
        }
        Console.WriteLine("|                                                                       |\n" +
                          "|\u001b[31m[<-]\u001b[0m                \u001b[34mBack(b)\u001b[0m" +
        ...
        switch (keyInfo.Key)
        {
            case ConsoleKey.B:
                ...
                break;
            case ConsoleKey.S:
                DisplayStats(character, Enemies.questEnemies[Arrow-1]);
                DisplayCharacterTurn(character, characterCollection);
                break;
            case ConsoleKey.Q:
                if (character.Skill1.CooldownLeft == 0 && Enemies.questEnemies[Arrow-1].IsAlive)
                {
                    DoesCharacterCooldown(character);
                    character.Skill1.CooldownLeft = character.Skill1.Cooldown;
                    AttackEnemy(Enemies.questEnemies[Arrow-1],character,1, characterCollection);
                }
                else
                {
                    DisplayCharacterTurn(character, characterCollection);
                ...
                break;
            case ConsoleKey.W:
                if (character.Skill2.CooldownLeft == 0 && Enemies.questEnemies[Arrow-1].IsAlive)
                {
                    DoesCharacterCooldown(character);
                    character.Skill2.CooldownLeft = character.Skill2.Cooldown;
                    AttackEnemy(Enemies.questEnemies[Arrow-1],character,2, characterCollection);
                }
                else
                {
                    DisplayCharacterTurn(character, characterCollection);
                ...
                break;
            case ConsoleKey.R:
                if (character.UltSkill.CooldownLeft == 0 && Enemies.questEnemies[Arrow-1].IsAlive)
                {
                    DoesCharacterCooldown(character);
                    character.UltSkill.CooldownLeft = character.UltSkill.Cooldown;
                    AttackEnemy(Enemies.questEnemies[Arrow-1],character,3, characterCollection);
                }
                else
                {
                    DisplayCharacterTurn(character, characterCollection);
            ...
            case ConsoleKey.DownArrow:
                if (Enemies.questEnemies[0].Tag == "BOSS")
                    Arrow = 1;
                else if (Arrow == 4)
                    Arrow = 1;
                else
        ...
        else if (!stun)
        {
            if (skill == 1) // Skill 1 
            {
                if (character.Skill1.BuffEffect == "" && character.Skill1.DebuffEffect == "") //NORMAL ATK
                {
                    ...
                    {
                        foreach (Enemies enemy in Enemies.questEnemies) //AOE
                        {
            ...
            for (int i = 0; i < Enemies.questEnemies.Count; i++)
            {
                if ((Enemies.questEnemies[i].HP>0) != tempKilledEnemiesList[i])
                {
                    if (Enemies.questEnemies[i].Tag=="BOSS")
